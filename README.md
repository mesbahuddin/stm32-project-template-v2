...
- [Usage](#usage)
  ...
  - [Automated Project Migration](#automated-project-migration)
...
- [GCC for ARM](https://developer.arm.com/downloads/-/gnu-rm) (GNU Arm Embedded
  Toolchain) is installed and available in your PATH. If not on PATH, set
  the `ARM_GCC_PATH` environment variable or pass `-DARM_GCC_PATH=/path/to/bin`
  to CMake.
...
This repository includes a powerful Python skill/script designed to automatically
migrate standard STM32CubeMX generated projects into this modular, CMake-based
architecture. 

If you have a new project generated by STM32CubeMX (using the Makefile or 
STM32CubeIDE toolchain setting), you can migrate it instantly:

```shell
python scripts/migrate_project.py /path/to/your/new_project
```

The script will automatically:
- Detect your MCU family and configure the CMake toolchain.
- Isolate vendor code (CMSIS, HAL, Middlewares like FatFs and USB Device) into a `lib/` STATIC library.
- Reorganize all your source code into `src/bsp/core/`, `src/bsp/brd/`, and `src/app/`.
- Inject the CMake build system, VS Code configs, and linting rules.
- Clean up unused template MCU files (e.g. removes `stm32l4-gcc.cmake` when
  targeting other families like STM32F4 or STM32U3).
...
- `src/bsp/startup/` — Startup code and vector table
- `src/bsp/core/` — MCU core peripheral drivers (GPIO, RCC, UART, SysTick, interrupt handlers, etc.)
- `src/bsp/brd/` — Board-specific component drivers and hardware glue (LED, Button, etc.)
...
The `scripts` folder contains helper scripts related to the project and the build
system.
